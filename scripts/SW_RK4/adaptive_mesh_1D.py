...
def monitor(x,sol_dx,alpha,arc_length):
...
def equid_mesh(N,mesh,source_dx_str,alpha,n_iter,arc_length=1):
    
    
    # equidistribute along y direction 
    y = mesh.coordinates()[:,1][0::N+1]
    
    source_dx = eval('lambda x: ' + source_dx_str)

    for i in range(n_iter):
        
        sol_dx = source_dx(y)
        rho = monitor(y,sol_dx,alpha,1)
